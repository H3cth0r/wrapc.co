...
from wrapcco.walkers.CPPLexer import CPPLexer
from wrapcco.walkers.CPPParser import CPPParser
from wrapcco.listeners.cpp_listener import CppListener

class ParserHandlerCpp:
    def __init__(self, filePath):
        self.filePath = filePath
        with open(filePath, "r") as fileContent: inputStream = InputStream(fileContent.read())
        lexer           = CPPLexer(inputStream)
        tokenStream     = CommonTokenStream(lexer)
        parser          = CPPParser(tokenStream)
        tree            = parser.program()
        self.listener   = CppListener()
